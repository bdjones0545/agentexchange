...
| `20260918_billing_accounts.sql` | Agent-card payments: `billing_accounts` (Stripe customer + saved card, written by the platform; operator may change only `agent_daily_cap_cents`, default $1,000/day) and `payments.authorized_by` (`human` at Checkout or `agent` off-session). |
