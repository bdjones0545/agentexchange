...
describe('agent owner setup',()=>{
 ...
 it('dedicated pending setup overrides a ready shared card',async()=>{const d=db();await d.from('agent_payment_cards').insert({key_id:key,profile_id:owner,mode:'dedicated',payment_method_id:null});const r=await setupStatus(d,owner,key,true,ready);expect(r.cardSource).toBe('dedicated');expect(r.canPay).toBe(false);});
